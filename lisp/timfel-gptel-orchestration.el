...
(declare-function agent-shell-rename-buffer "agent-shell" (name &optional unique))
...
(declare-function ci--request-with-proxy-aware-settings "emacs-ci" (full-url &rest settings))
...
(declare-function jira-api-search "jira-api" (&key params callback sync errback))
(declare-function request-abort "request" (response))
(declare-function request-response-p "request" (cl-x))
